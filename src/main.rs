...
use axum::{extract::Path, http::StatusCode, routing::{get, put}, Router};
...
async fn main() {
    ...
    let git_repository_path = PathBuf::from(git_repository_from_env).canonicalize().unwrap();
    ...
    let router: Router = Router::new()
        ...
        .route("/api/v1/crates/:crate_name/:version/download", get(download_handler))
...
async fn download_handler(
    Path(DownloadPath { crate_name, version }): Path<DownloadPath>
) -> Result<Vec<u8>, (StatusCode, &'static str)> {
    get_crate_file(version, &crate_name)
        .await
        .map_err(|e| match e {
        e if e.kind() == std::io::ErrorKind::NotFound => (StatusCode::NOT_FOUND, "crate or version doesn't exist"),
        _ => (StatusCode::INTERNAL_SERVER_ERROR, "couldn't get crate file for you"),
    })
}
