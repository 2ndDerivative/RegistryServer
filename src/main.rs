use std::{fmt::{Display, Formatter, Result as FmtResult}, net::{IpAddr, SocketAddr}, path::PathBuf, sync::Arc};

use axum::{body::{to_bytes, Body}, http::{header::CONTENT_LENGTH, HeaderMap, StatusCode}, response::{IntoResponse, Response}, routing::put, Router};
...
async fn publish_handler(headers: HeaderMap, body: Body) -> Result<String, Response> {
    let content_length: Option<usize> = headers.get(CONTENT_LENGTH)
        .map(|b| b.to_str().unwrap().parse().unwrap());
    let body_bytes = to_bytes(body, content_length.unwrap_or(usize::MAX)).await.unwrap();
    let (metadata_length_bytes, rest) = body_bytes.split_first_chunk::<4>()
        .ok_or(PublishError::UnexpectedEOF.into_response())?;
    let metadata_length = u32::from_le_bytes(*metadata_length_bytes) as usize;
    let (metadata_bytes, request_body_rest) = rest.split_at_checked(metadata_length)
        .ok_or(PublishError::UnexpectedEOF.into_response())?;
    let (file_length_bytes, file_content) = request_body_rest.split_first_chunk()
        .ok_or(PublishError::UnexpectedEOF.into_response())?;
